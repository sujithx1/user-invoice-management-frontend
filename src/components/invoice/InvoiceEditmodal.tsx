...
import type { UpdateInvoiceRequest, Invoice, ValidationError } from "../../types"
...
interface EditInvoiceModalProps {
  ...
  onClose: () => void
  onUpdateInvoice: (invoiceId: string, invoiceData: UpdateInvoiceRequest) => void
...
export default function EditInvoiceModal({
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.invoiceNumber && !/^\d+$/.test(formData.invoiceNumber)) {
    ...
    if (formData.dueDate && formData.invoiceDate && formData.dueDate < formData.invoiceDate) {
      newErrors.push({ field: "dueDate", message: "Due date cannot be before invoice date" })
    }

    // Invoice-specific validations
    if (formData.invoiceNumber && formData.invoiceDate && invoice) {
      const validation = validateInvoiceDate(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdateInvoice(invoice.invoiceId, formData)
      handleClose()
    } catch (error) {
...
const handleInputChange = <K extends keyof UpdateInvoiceRequest>(
  field: K,
  value: UpdateInvoiceRequest[K]
) => {
  setFormData((prev) => ({
    ...prev,
    [field]: value,
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-xl">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xl font-bold text-gray-900">Edit Invoice #{invoice.invoiceNumber}</h3>
